...
function bloquearScroll() {
    window.addEventListener("wheel", evitarScroll, { passive: false });
}

function desbloquearScroll() {
    window.removeEventListener("wheel", evitarScroll);
}

function evitarScroll(evento) {
    evento.preventDefault();
}

function scrollFluido(destino, duracion = 1000) {
...
btnVerMas.addEventListener("click", () => {
    ...
    if (wrapperExtra.classList.contains("visible")) {
      ...
      setTimeout(() => {
        const destino = wrapperExtra.getBoundingClientRect().top +
        ...
        const destino = wrapperExtra.getBoundingClientRect().top + window.pageYOffset - 150;
